...
#### definition of drainage capacity function - it compares the drainage capacity from pumps with that from channels 
def drain_cap (h_tide, h_store, h_min = -2000, pump_par = pumpcap_fit, channel_par = [1.016 , 2572.], dh = 50, wind_safe = 0, gradmax = 4000):
    ''' This function tries to find the maximal flow rate in a system with a pump and a canal.
    ...
    h_tide :: the outer water level at the pump
    h_store :: the canal water level
    h_min :: the lower boundary of the inner water level
    pump_par :: parameters of the pump function
    channel_par :: parameters of the canal flow function
    dh :: increment of inner water level estimation
    wind_safe :: gradient from canal to inner water level, which is induced by wind and therefore not contributing to flow
    gradmax :: maximum gradient from inner to outer water level, at which pumps shall run 

    returns an array with [0] the actual maximum flow through canal and pump, [1] the estimated inner water level and [2] the maximum flow, which could be pumped if not limited by canals
    '''
...
#        print("q_pump: "+str(q_pump))
#        print("q_channel: "+str(q_channel))
